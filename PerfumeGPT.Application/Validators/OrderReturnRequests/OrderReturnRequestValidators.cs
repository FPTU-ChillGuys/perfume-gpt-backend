...
namespace PerfumeGPT.Application.Validators.OrderReturnRequests
{
	...
	public class ProcessInStoreReturnFastTrackDtoValidator : AbstractValidator<ProcessInStoreReturnFastTrackDto>
	{
		public ProcessInStoreReturnFastTrackDtoValidator()
		{
			RuleFor(x => x.OrderId).NotEmpty().WithMessage("ID đơn hàng là bắt buộc.");
			RuleFor(x => x.OrderCode).NotEmpty().WithMessage("Mã đơn hàng là bắt buộc.");
			RuleFor(x => x.CustomerNote)
				.MaximumLength(1000).WithMessage("Ghi chú của khách hàng không được vượt quá 1000 ký tự.")
				.When(x => !string.IsNullOrWhiteSpace(x.CustomerNote));
			RuleFor(x => x.InspectionNote)
				.MaximumLength(1000).WithMessage("Ghi chú kiểm định không được vượt quá 1000 ký tự.")
				.When(x => !string.IsNullOrWhiteSpace(x.InspectionNote));
			RuleFor(x => x.ApprovedRefundAmount)
				.GreaterThanOrEqualTo(0).WithMessage("Số tiền hoàn đề xuất phải lớn hơn hoặc bằng 0.");
			RuleFor(x => x.ReturnItems).NotEmpty().WithMessage("Ít nhất một mục trả hàng là bắt buộc.");
			RuleForEach(x => x.ReturnItems)
				.ChildRules(item =>
				{
					item.RuleFor(i => i.OrderDetailId).NotEmpty().WithMessage("ID chi tiết đơn hàng là bắt buộc.");
					item.RuleFor(i => i.Quantity).GreaterThan(0).WithMessage("Số lượng trả hàng phải lớn hơn 0.");
				});
			RuleFor(x => x.ReturnItems)
				.Must(items => items == null || items.GroupBy(i => i.OrderDetailId).All(g => g.Count() == 1))
				.WithMessage("Các mục trả hàng không được chứa ID chi tiết đơn hàng trùng lặp.");
			...
			RuleFor(x => x)
				.Must(x =>
				{
					var hasBank = !string.IsNullOrWhiteSpace(x.RefundBankName)
						|| !string.IsNullOrWhiteSpace(x.RefundAccountNumber)
						|| !string.IsNullOrWhiteSpace(x.RefundAccountName);
