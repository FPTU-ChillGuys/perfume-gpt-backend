	...
	public class OrderFulfillmentService : IOrderFulfillmentService
	{
		...
		public async Task<PickListResponse> GetPickListAsync(Order order)
		{
			...
			if (activeReservations.Count == 0)
				throw AppException.BadRequest("Không tìm thấy dữ liệu giữ chỗ (reservation) nào đang hiệu lực để tạo phiếu soạn hàng.");

			var pickListItems = await BuildPickListItemsAsync(order.OrderDetails, activeReservations);

			return new PickListResponse { OrderId = order.Id, Code = order.Code, Items = pickListItems };
		}

		private async Task<List<PickListItemResponse>> BuildPickListItemsAsync(ICollection<OrderDetail> orderDetails, List<StockReservation> activeReservations)
		{
			...
			var reservationsByOrderDetail = activeReservations.GroupBy(r => r.OrderDetailId);

			foreach (var group in reservationsByOrderDetail)
			{
				var orderDetailId = group.Key;
				var orderDetail = orderDetails.FirstOrDefault(od => od.Id == orderDetailId);
				if (orderDetail == null) continue;

				var totalQtyForDetail = group.Sum(r => r.ReservedQuantity);
		...
		private async Task<BaseResponse<bool>> ValidateScannedBatchCodesAsync(Order order, FulfillOrderRequest request)
		{
			...
			var groupedRequestItems = request.Items.GroupBy(i => i.OrderDetailId).ToList();

			foreach (var group in groupedRequestItems)
			{
				var orderDetailId = group.Key;
				var orderDetail = order.OrderDetails.FirstOrDefault(od => od.Id == orderDetailId)
					?? throw AppException.BadRequest($"Không tìm thấy chi tiết đơn hàng {orderDetailId}.");

				var detailReservations = activeReservations.Where(r => r.OrderDetailId == orderDetailId).ToList();

				var totalScannedQty = group.Sum(i => i.Quantity);
				var totalReservedQty = detailReservations.Sum(r => r.ReservedQuantity);

				if (totalScannedQty != totalReservedQty)
					throw AppException.BadRequest($"Tổng số lượng quét ({totalScannedQty}) không khớp số lượng yêu cầu ({totalReservedQty}) cho chi tiết đơn hàng {orderDetailId}.");

				var scannedTracker = group.GroupBy(i => i.ScannedBatchCode).ToDictionary(g => g.Key, g => g.Sum(i => i.Quantity));

				foreach (var res in detailReservations)
				{
					var batchCode = batchDictionary[res.BatchId].BatchCode;

					if (!scannedTracker.TryGetValue(batchCode, out var scannedQty) || scannedQty < res.ReservedQuantity)
						throw AppException.BadRequest($"Mã lô quét cho chi tiết {orderDetailId} không khớp. Yêu cầu lô '{batchCode}', số lượng {res.ReservedQuantity}.");

					scannedTracker[batchCode] -= res.ReservedQuantity;
				}
			}
		...
		public async Task<SwapDamagedStockResponse> SwapDamagedStockAsync(Guid orderId, Guid staffId, SwapDamagedStockRequest request)
		{
			...
			return await _unitOfWork.ExecuteInTransactionAsync(async () =>
			{
				...
				return new SwapDamagedStockResponse
				{
					...
					Message = $"Đổi {quantityToSwap} sản phẩm hỏng thành công. Hệ thống đã sử dụng {replacementAllocations.Count} lô thay thế khác."
				};
			});
		}
