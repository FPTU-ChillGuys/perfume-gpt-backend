...
namespace PerfumeGPT.Application.Services.Helpers.OrderHelpers
{
	public class OrderShippingHelper : IOrderShippingHelper
	{
		...
		public async Task SetupShippingInfoAsync(Order order, ContactAddressInformation? contactAddressRequest, Guid? customerId, Guid? savedAddressId, decimal shippingFee, DateTime? estimatedDeliveryDate)
		{
			// 1. Lưu Contact Address vào DB
			...
			// 2. Tạo Shipping Info
			var shippingInfo = ShippingInfo.Create(CarrierName.GHN, ShippingType.Forward, shippingFee, estimatedDeliveryDate);
